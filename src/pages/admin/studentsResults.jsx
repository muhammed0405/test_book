...
const StudentsResults = () => {
    ...
    // Жаңы функция: колдонуучуларга суроолорду жашыруу жана көрсөтүү
    const handleToggleUserQuestions = () => {
        if (window.confirm(isQuestionsVisible ? "Суроолорду жашырууга макулсузбу?" : "Суроолорду көрсөтүүгө макулсузбу?")) {
            try {
                setIsQuestionsVisible(!isQuestionsVisible); // Суроолордун көрүнүп жатканын өзгөртүү
            } catch (error) {
                alert("Ката чыкты, кайра аракет кылыңыз: " + error);
            }
        }
    ...
    if (error) return <div className="text-center py-4 text-red-600">Ката: {error}</div>;
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="flex flex-wrap justify-center gap-2 mb-6">
                ...
                >
                    {isQuestionsVisible ? "🙈 суроолорду жашырууга" : "👀 суроолорду көрсөтүүгө"}
                </button>
            </div>
                ...
                <ul className="space-y-4">
                    {sortedResults.map((studentResult, index) => (
                        <li
                            ...
                            className={`bg-white shadow-md rounded-lg p-6 hover:shadow-lg transition-shadow duration-300 ${!isQuestionsVisible ? 'hidden' : ''}`}
                        >
                            ...
                            <div className="mt-4 bg-red-500 rounded-full h-2">
                                <div
                                    ...
                                    style={{
                                        width: `${(studentResult.correctAnswers /
                                            (studentResult.correctAnswers +
                                                studentResult.wrongAnswers)) *
                                            100}%`,
                                    }}
